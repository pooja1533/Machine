...
namespace Hutech.Sql.Queries
{
    ...
    {
        public static string GetAuditTrail => " Select AuditId,ModuleName,IPAddress,Request_Data,Description,CreatedDatetime,a.UserName as UserId , (select STRING_AGG(r.name,',')from AspNetRoles r join AspNetUserRoles ar on ar.RoleId=r.Id where ar.UserId= au.UserID) as role " +

...
namespace Hutech.Infrastructure.Repository
{
    public class AuditTrailRepository : IAuditTrailRepository
    {
        ...
        public async Task<ExecutionResult<GridData<Audit>>> GetUserAuditTrail(string startDate, string endDate, string keyword, int pageNumber,string userId)
        {
            try
            {
                int maxRows = 10;
                using (IDbConnection connection = new SqlConnection(configuration.GetConnectionString("DBConnection")))
                {
                    ...
                    var recordsPerPage = 10;
                    var skipRecords = (pageNumber - 1) * recordsPerPage;
                    var result = await connection.QueryAsync<Audit>(AuditQueries.GetUserAuditTrail, new { fromDate = startDate, toDate = endDate, keyword = keyword,userId=userId });
                    if (pageNumber > 0)
                    {
                        var totalRecords = result.Count();
